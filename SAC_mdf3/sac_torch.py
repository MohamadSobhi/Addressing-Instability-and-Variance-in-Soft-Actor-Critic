...
class Agent():
    def __init__(self, alpha=0.0003, beta=0.0003, input_dims=[8],
            env=None, gamma=0.99, update_actor_interval=2, warmup=1000, n_actions=2, max_size=1000000, tau=0.005,
            layer1_size=400, layer2_size=300, batch_size=256, reward_scale=2, noise=0.1):
        self.gamma = gamma
        self.tau = tau
    ...
    def choose_action(self, observation):
        ...
        #     actions = T.tensor(np.random.normal(scale=self.noise,size=(self.n_actions,))).to(self.actor.device) + T.tensor(np.random.normal(scale=self.noise),
        #                                               dtype=T.float).to(self.actor.device)
        # else:
        state = T.Tensor([observation]).to(self.actor.device)
        actions, _ = self.actor.sample_normal(state, reparameterize=False)
        actions = actions + T.tensor(np.random.normal(scale=self.noise),
                                                      dtype=T.float).to(self.actor.device)

        actions = T.clamp(actions, self.min_action[0], self.max_action[0])

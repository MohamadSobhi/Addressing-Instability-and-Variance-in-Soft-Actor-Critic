...
class Agent():
    def __init__(self, alpha=0.0003, beta=0.0003, input_dims=[8],
            env=None, gamma=0.99, update_actor_interval=2, n_actions=2, max_size=1000000, tau=0.005,
            layer1_size=400, layer2_size=300, batch_size=256, reward_scale=2):
        self.gamma = gamma
        self.tau = tau
        ...
        self.critic_2 = CriticNetwork(beta, input_dims, n_actions=n_actions, fc1_dims=layer1_size, fc2_dims=layer2_size,
                    name='critic_2')
        self.critic_3 = CriticNetwork(beta, input_dims, n_actions=n_actions, fc1_dims=layer1_size, fc2_dims=layer2_size,
                    name='critic_3')
    ...
    def learn(self):
        ...
        q3_old_policy = self.critic_3.forward(state, action).view(-1)
        critic_1_loss = 0.5 * F.mse_loss(q1_old_policy, q_hat)
        critic_2_loss = 0.5 * F.mse_loss(q2_old_policy, q_hat)
        critic_3_loss = 0.5 * F.mse_loss(q3_old_policy, q_hat)

        critic_loss = critic_1_loss + critic_2_loss + critic_3_loss
        ...
        q1_new_policy = self.critic_1.forward(state, actions)
        q2_new_policy = self.critic_2.forward(state, actions)
        q2_new_policy = self.critic_3.forward(state, actions)
        critic_value = T.min(q1_new_policy, q2_new_policy)
        critic_value = critic_value.view(-1)
